...
contract TokenVault is ITokenVault, AccessControlEnumerable, RevestAccessControl {
    ...
    mapping(uint256 => IRevest.FNFTConfig) private fnfts;
    mapping(address => IRevest.TokenTracker) public tokenTrackers;

    uint256 private constant multiplierPrecision = 1 ether;
     ...
     */
    function updateBalance(uint256 fnftId, uint256 incomingDeposit) internal {
        ...
        uint256 lastBal = tracker.lastBalance;
     ...
     */
    function depositToken(uint256 fnftId, uint256 transferAmount, uint256 quantity)
        public
        override
    ...
    function withdrawToken(uint256 fnftId, uint256 quantity, address user) external override onlyRevestController {
        IRevest.FNFTConfig storage fnft = fnfts[fnftId];
        ...
        uint256 withdrawAmount = fnft.depositAmount * quantity * tracker.lastMul / fnft.depositMul;
    ...
    function removeFNFT(uint256 fnftId) internal {
        delete fnfts[fnftId];
    }

    // amount = amount per vault for new mapping
    function handleMultipleDeposits(uint256 fnftId, uint256 newFNFTId, uint256 amount)
        external
        override
        onlyRevestController
    {
     ...
     *
     */
    function getFNFT(uint256 fnftId) external view override returns (IRevest.FNFTConfig memory) {
        return fnfts[fnftId];
    }
}
